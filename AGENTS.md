...
- Set `insecure_tls = False` and supply `cert_fingerprint` for MITM protection. Camera TLS (port 6000) **requires** a pin (or `insecure_tls`) and fails closed without one. MQTT/FTPS without a pin use system CA verification (`CERT_REQUIRED`), which fails for typical Bambu self-signed certs — still effectively fail-closed, but prefer an explicit pin. Pin match/mismatch is enforced when a fingerprint is configured.
