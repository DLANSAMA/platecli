...
def test_detect_orca_returns_first_existing():
    with patch.object(config, "_orca_binary_candidates", return_value=["/nope/a", "/found/orca", "/other"]), \
...
def test_detect_profiles_returns_first_existing_dir():
    with patch.object(config, "_profiles_dir_candidates", return_value=["/nope", "/found/BBL"]), \
...
def test_preflight_suggests_detected_orca_when_configured_path_bad():
    ...
    with patch("bambu_cli.setup_cmd.preflight.load_config", return_value=cfg), \
         ...
         patch("bambu_cli.slicer._slicer_executable_problem", return_value="OrcaSlicer not found at /bad/orca"), \
